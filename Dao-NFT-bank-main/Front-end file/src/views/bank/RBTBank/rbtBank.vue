<template>
  <div class="rbt-bank">
    <LokeshJOshi class="rainCard-box">
...
export default {
  ...
  methods: {
    ...
    withdraw(item) {
      this.$store.dispatch("LokeshJOshi/withdrawa", {
        to: this.account,
        tokenId: item.index
      }).then(() => {
    ...
    release(tokenId) {
      this.$store.dispatch("LokeshJOshi/withdrawa", {to: this.$store.state.app.account, tokenId}).then(() => {
    ...
    subPledge() {
      this.$store.dispatch("LokeshJOshi/depositToken", {
        ...
        let arr = this.$store.state.LokeshJOshi.records
        arr.push({
          month: this.chooseMonth,
